...
//选择排序
void Sort::selectSort(int* a, int size)
{
	int max_idx;
	for(int i=size-1;i>0;i--)
	{
...
//希尔排序
void Sort::hellSort(int* a, int size)
{
	if(size < 2)
	{
		return;
...
//堆排序，大顶堆
void Sort::maxHeapSort(int* a, int size)
{
	//构建大顶堆，从最后一个父节点开始
...
void Sort::maxHeapify(int* a, int root, int size)
{
	int lchild = 2*root+1;
	if(lchild >= size)
	{
	...
	int rchild = lchild+1;
	if(rchild < size && a[rchild]>a[max_child])
	{
		max_child = rchild;
	}
